...
HttpServletResponse response)
		...
		try {
			...
			if(option==1) {
				...
				rs = st.executeQuery();
				out.println("<table class='tb' border=1 width=50% height=50%>");
				out.println("<tr><th>Project_Id</th><th>Title</th><th>Mentor</th><th>Project Description</th><th>Student_id</th></tr>");
				while(rs.next()) {
					...
					int sid = rs.getInt("s_id");
					 out.println("<tr><td>" + pid + "</td><td>" + title + "</td><td>" + mentor+ "</td><td>"+ prodesc+"</td><td>"+ sid +"</td></tr>");
				}
				out.println("</table>");
			}
