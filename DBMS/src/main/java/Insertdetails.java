	...
	protected void service(HttpServletRequest request,
		...
		try {
			...
			if(option==1) {
				...
				rd.forward(request, response);
				research(request,response, out, con);
			}
			else if(option == 3) {
				rd = request.getRequestDispatcher("interninput.html");
				rd.forward(request, response);
				internship(request,response, out, con);
			}
			else if(option == 4) {
				rd = request.getRequestDispatcher("projectinput.html");
				rd.forward(request, response);
