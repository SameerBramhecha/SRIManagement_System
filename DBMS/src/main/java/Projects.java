...
HttpServletResponse response)
		...
		try {
			...
			ResultSet rs = null;
			if(domain.equals("")) {
				st = con.prepareStatement("select * from projects p inner join project_domain pd on p.project_id = pd.project_id ;");
				rs = st.executeQuery();
			}
			else{
			...
			out.println("<tr><th>Project_Id</th><th>Title</th><th>Mentor</th><th>Project Description</th><th>Student_id</th><th>Domain</th></tr>");
			while(rs.next()) {
				...
				String prodomain = rs.getString("domain");
				 out.println("<tr><td>" + pid + "</td><td>" + title + "</td><td>" + mentor+ "</td><td>"+ prodesc+"</td><td>"+ sid +"</td><td>"+ prodomain+"</td></tr>");
			}
			out.println("</table>");	
